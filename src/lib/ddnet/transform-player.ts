/**
 * Client-side transform: converts raw API response {player, skin}
 * into the shape expected by +page.svelte.
 *
 * Extracted from the old +page.ts (SSR data transform).
 */
export function transformPlayerData(data: { player: any; skin: any }) {
	...
	let maps = Object.keys(player.types)
		.flatMap((type: string) =>
			Object.entries(player.types[type].maps as Record<string, any>).map(([name, map]) => {
				if (map.points == 0) {
					map.rank = undefined;
