...
<div class="mx-auto mt-5 max-w-4xl">
	<div class="text-center text-slate-300">
		...
		<!-- Guild Selection -->
		<div class="mt-4">
			<label for="guild-select" class="mb-2 block text-left">选择频道:</label>
			<select
				id="guild-select"
		...
		<!-- Error Display -->
		{#if error}
			<div class="mt-4 rounded bg-red-800 p-3 text-white">
				{error}
			</div>
		{/if}
		...
		<!-- Role Form -->
		{#if showRoleForm}
			<div class="mt-6 rounded-lg p-4" style="background-color: #101010;">
				<h2 class="mb-4 text-xl font-bold">{editingRole ? '编辑角色' : '创建新角色'}</h2>
				<form
					onsubmit={(e) => {
						e.preventDefault();
						submitForm();
				...
				>
					<div class="mb-4">
						<label for="role-name" class="mb-1 block">角色名称:</label>
						<input
							id="role-name"
							type="text"
					...
					<div class="mb-4">
						<label for="role-color" class="mb-1 block">角色颜色:</label>
					...
					<div class="flex justify-end gap-2">
						<button
							type="button"
							class="rounded bg-slate-600 px-4 py-2 hover:bg-slate-500"
							onclick={cancelEdit}
						>
							取消
						</button>
						<button
							type="submit"
							class="rounded bg-sky-700 px-4 py-2 hover:bg-sky-600"
							disabled={loading}
						>
		...
		{:else if selectedGuildId}
			<div class="mt-4">
				<button class="rounded bg-sky-700 px-4 py-2 hover:bg-sky-600" onclick={startNewRole}>
					创建新角色
				</button>
			</div>
		{/if}
		...
		{#if selectedGuildId}
			<div class="mt-6 rounded-lg p-4" style="background-color: #101010;">
				<h2 class="mb-4 text-left text-xl font-bold">角色列表</h2>
							...
							<tbody>
								{#each roles as role}
									<tr class="border-t border-zinc-800 hover:bg-zinc-900">
										...
										<td class="p-2">
											<span style="color: {colorToHex(role.color).substring(0, 7)};">
												{role.name}
											</span>
										</td>
										<td class="p-2">
										...
										<td class="p-2">
											...
											{#if !['1', '2', '4', '5', '6'].includes(role.id)}
												<div class="flex gap-2">
													<button
														class="rounded bg-sky-700 px-2 py-1 text-sm hover:bg-sky-600"
														onclick={() => editRole(role)}
													>
														编辑
													</button>
													<button
														class="rounded bg-red-700 px-2 py-1 text-sm hover:bg-red-600"
